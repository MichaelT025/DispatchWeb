...
import { AGENT_ROLES, hasDispatchExtension, type AgentRole } from "../../web/src/agents.js";
...
describe("AgentPicker", () => {
	...
	// `hasDispatchExtension` gate into the picker so a partial catalog or a
	...
	it("hides role buttons when only a partial extension catalog is present", () => {
		const available = hasDispatchExtension([{ name: "agent", source: "extension" }]);
	...
	it("hides role buttons when a prompt template collides with an extension name", () => {
		const available = hasDispatchExtension([
